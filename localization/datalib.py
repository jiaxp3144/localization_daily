...
def addNoise(sig, params, snr, noise_file_path):
  ''' NOT recommand! try to use addNoiseFromSig instead!
...
def addNoiseFromSig(sig, params, snr, noise_sig):
  ''' MAKE sure the sig and noise_sig have the same sampling rate!
      add noise according to the noise type and snr
  '''
  # if dual channel
  if params[0]==2:
    ...
    sig_r = sig[:,0]
    sig_l = sig[:,1]
    sig_r_n = addNoiseFromSig(sig_r,params_single,snr,noise_sig)
    sig_l_n = addNoiseFromSig(sig_l,params_single,snr,noise_sig)
    return np.array([sig_r_n, sig_l_n]).T
  # if single channel
  else:
    # random to start index
    noise_len = len(noise_sig)
    sig_len = len(sig)
    if noise_len <= 5*sig_len:
      print('ERROR: noise file length is too short for the signal!')
      return
    start_index = random.randint(0, noise_len-len(sig))
    noise_slice = noise_sig[start_index:start_index+sig_len]
    # decided by snr
    signal_power = np.sum(sig**2)/sig_len
    noise_var = signal_power/(10**(snr/10.0))
    noise_slice = np.sqrt(noise_var)/np.std(noise_slice)*noise_slice
